// SPDX-FileCopyrightText: 2024 The Crossplane Authors <https://crossplane.io>
//
// SPDX-License-Identifier: Apache-2.0

package controller

import (
	ctrl "sigs.k8s.io/controller-runtime"

	"github.com/crossplane/upjet/v2/pkg/controller"

	release "github.com/upbound/provider-gcp/v2/internal/controller/namespaced/firebaserules/release"
	ruleset "github.com/upbound/provider-gcp/v2/internal/controller/namespaced/firebaserules/ruleset"
)

// Setup_firebaserules creates all controllers with the supplied logger and adds them to
// the supplied manager.
func Setup_firebaserules(mgr ctrl.Manager, o controller.Options) error {
	for _, setup := range []func(ctrl.Manager, controller.Options) error{
		release.Setup,
		ruleset.Setup,
	} {
		if err := setup(mgr, o); err != nil {
			return err
		}
	}
	return nil
}

// SetupGated_firebaserules creates all controllers with the supplied logger and adds them to
// the supplied manager gated.
func SetupGated_firebaserules(mgr ctrl.Manager, o controller.Options) error {
	for _, setup := range []func(ctrl.Manager, controller.Options) error{
		release.SetupGated,
		ruleset.SetupGated,
	} {
		if err := setup(mgr, o); err != nil {
			return err
		}
	}
	return nil
}

// SetupWebhookWithManager_firebaserules registers conversion webhooks for all resource kinds in the group.
func SetupWebhookWithManager_firebaserules(mgr ctrl.Manager) error {
	for _, setup := range []func(ctrl.Manager) error{
		release.SetupWebhookWithManager,
		ruleset.SetupWebhookWithManager,
	} {
		if err := setup(mgr); err != nil {
			return err
		}
	}
	return nil
}
