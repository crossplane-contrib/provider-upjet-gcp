// SPDX-FileCopyrightText: 2024 The Crossplane Authors <https://crossplane.io>
//
// SPDX-License-Identifier: Apache-2.0

package controller

import (
	ctrl "sigs.k8s.io/controller-runtime"

	"github.com/crossplane/upjet/v2/pkg/controller"

	sshpublickey "github.com/upbound/provider-gcp/v2/internal/controller/cluster/oslogin/sshpublickey"
)

// Setup_oslogin creates all controllers with the supplied logger and adds them to
// the supplied manager.
func Setup_oslogin(mgr ctrl.Manager, o controller.Options) error {
	for _, setup := range []func(ctrl.Manager, controller.Options) error{
		sshpublickey.Setup,
	} {
		if err := setup(mgr, o); err != nil {
			return err
		}
	}
	return nil
}

// SetupGated_oslogin creates all controllers with the supplied logger and adds them to
// the supplied manager gated.
func SetupGated_oslogin(mgr ctrl.Manager, o controller.Options) error {
	for _, setup := range []func(ctrl.Manager, controller.Options) error{
		sshpublickey.SetupGated,
	} {
		if err := setup(mgr, o); err != nil {
			return err
		}
	}
	return nil
}

// SetupWebhookWithManager_oslogin registers conversion webhooks for all resource kinds in the group.
func SetupWebhookWithManager_oslogin(mgr ctrl.Manager) error {
	for _, setup := range []func(ctrl.Manager) error{
		sshpublickey.SetupWebhookWithManager,
	} {
		if err := setup(mgr); err != nil {
			return err
		}
	}
	return nil
}
