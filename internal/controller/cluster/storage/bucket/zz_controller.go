...
import (
	...
	v1beta3 "github.com/upbound/provider-gcp/v2/apis/cluster/storage/v1beta3"
...
func SetupGated(mgr ctrl.Manager, o tjcontroller.Options) error {
	o.Options.Gate.Register(func() {
		if err := Setup(mgr, o); err != nil {
			mgr.GetLogger().Error(err, "unable to setup reconciler", "gvk", v1beta3.Bucket_GroupVersionKind.String())
		}
	}, v1beta3.Bucket_GroupVersionKind)
	return nil
}

// Setup adds a controller that reconciles Bucket managed resources.
func Setup(mgr ctrl.Manager, o tjcontroller.Options) error {
	name := managed.ControllerName(v1beta3.Bucket_GroupVersionKind.String())
	var initializers managed.InitializerChain
	initializers = append(initializers, managed.NewNameAsExternalName(mgr.GetClient()))
	rl := reconciliationpolicy.NewExponentialFailureRateLimiter(time.Second, 60*time.Second)
	eventHandler := handler.NewEventHandler(handler.WithDefaultRateLimiter(rl), handler.WithLogger(o.Logger.WithValues("gvk", v1beta3.Bucket_GroupVersionKind)))
	ac := tjcontroller.NewAPICallbacks(mgr, xpresource.ManagedKind(v1beta3.Bucket_GroupVersionKind), tjcontroller.WithEventHandler(eventHandler), tjcontroller.WithStatusUpdates(false))
	opts := []managed.ReconcilerOption{
		managed.WithExternalConnecter(
			tjcontroller.NewTerraformPluginSDKAsyncConnector(mgr.GetClient(), o.OperationTrackerStore, o.SetupFn, o.Provider.Resources["google_storage_bucket"],
				...
				tjcontroller.WithTerraformPluginSDKAsyncMetricRecorder(metrics.NewMetricRecorder(v1beta3.Bucket_GroupVersionKind, mgr, o.PollInterval)),
	...
	// register webhooks for the kind v1beta3.Bucket if they're enabled.
	if o.StartWebhooks {
		if err := ctrl.NewWebhookManagedBy(mgr, &v1beta3.Bucket{}).
			Complete(); err != nil {
			return errors.Wrap(err, "cannot register webhook for the kind v1beta3.Bucket")
		}
	}

	if o.MetricOptions != nil && o.MetricOptions.MRStateMetrics != nil {
		stateMetricsRecorder := statemetrics.NewMRStateRecorder(
			mgr.GetClient(), o.Logger, o.MetricOptions.MRStateMetrics, &v1beta3.BucketList{}, o.MetricOptions.PollStateMetricInterval,
		)
		if err := mgr.Add(stateMetricsRecorder); err != nil {
			return errors.Wrap(err, "cannot register MR state metrics recorder for kind v1beta3.BucketList")
		}
	}

	r := managed.NewReconciler(mgr, xpresource.ManagedKind(v1beta3.Bucket_GroupVersionKind), opts...)
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		Watches(&v1beta3.Bucket{}, eventHandler).
		Complete(
