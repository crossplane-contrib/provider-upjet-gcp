	...
	for i3 := 0; i3 < len(mg.Spec.ForProvider.Subnet); i3++ {
		{
			...
			rsp, err = r.Resolve(ctx, reference.ResolutionRequest{
				CurrentValue: ptr.Deref(mg.Spec.ForProvider.Subnet[i3].Name, ""),
				Extract:      reference.ExternalName(),
		...
		mg.Spec.ForProvider.Subnet[i3].Name = ptr.To(rsp.ResolvedValue)
	...
	for i3 := 0; i3 < len(mg.Spec.InitProvider.Subnet); i3++ {
		{
			...
			rsp, err = r.Resolve(ctx, reference.ResolutionRequest{
				CurrentValue: ptr.Deref(mg.Spec.InitProvider.Subnet[i3].Name, ""),
				Extract:      reference.ExternalName(),
		...
		mg.Spec.InitProvider.Subnet[i3].Name = ptr.To(rsp.ResolvedValue)
