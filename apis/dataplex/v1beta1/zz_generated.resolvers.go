...
func (mg *LakeIAMPolicy) ResolveReferences(ctx context.Context, c client.Reader) error {
	...
	mg.Spec.ForProvider.ProjectRef = rsp.ResolvedReference
	{
		...
		rsp, err = r.Resolve(ctx, reference.ResolutionRequest{
			CurrentValue: reference.FromPtrValue(mg.Spec.InitProvider.Lake),
			Extract:      reference.ExternalName(),
			Reference:    mg.Spec.InitProvider.LakeRef,
			Selector:     mg.Spec.InitProvider.LakeSelector,
			To:           reference.To{List: l, Managed: m},
		})
	}
	if err != nil {
		return errors.Wrap(err, "mg.Spec.InitProvider.Lake")
	}
	mg.Spec.InitProvider.Lake = reference.ToPtrValue(rsp.ResolvedValue)
	mg.Spec.InitProvider.LakeRef = rsp.ResolvedReference
	{
		m, l, err = apisresolver.GetManagedResource("dataplex.gcp.upbound.io", "v1beta2", "Lake", "LakeList")
		if err != nil {
			return errors.Wrap(err, "failed to get the reference target managed resource and its list for reference resolution")
