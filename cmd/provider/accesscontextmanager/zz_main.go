...
func main() {
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_accesscontextmanager(mgr, clusterOpts), "Cannot setup cluster-scoped AzureAD controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_accesscontextmanager(mgr, namespacedOpts), "Cannot setup namespaced AzureAD controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_accesscontextmanager(mgr, clusterOpts), "Cannot setup cluster-scoped AzureAD controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_accesscontextmanager(mgr, namespacedOpts), "Cannot setup namespaced AzureAD controllers")
	}
