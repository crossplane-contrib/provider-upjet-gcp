...
func main() {
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_oslogin(mgr, clusterOpts), "Cannot setup cluster-scoped AzureAD controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_oslogin(mgr, namespacedOpts), "Cannot setup namespaced AzureAD controllers")
	} else {
		logr.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(clustercontroller.Setup_oslogin(mgr, clusterOpts), "Cannot setup cluster-scoped AzureAD controllers")
		kingpin.FatalIfError(namespacedcontroller.Setup_oslogin(mgr, namespacedOpts), "Cannot setup namespaced AzureAD controllers")
	}
