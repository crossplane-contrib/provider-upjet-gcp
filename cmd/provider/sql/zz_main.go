...
func main() {
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(clustercontroller.SetupGated_sql(mgr, clusterOpts), "Cannot setup cluster-scoped AzureAD controllers")
		kingpin.FatalIfError(namespacedcontroller.SetupGated_sql(mgr, namespacedOpts), "Cannot setup namespaced AzureAD controllers")
	} else {
